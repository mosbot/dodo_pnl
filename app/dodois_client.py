...
# Параллелизм per-unit запросов. 12 — компромисс между мелкими сетями
# (PiX 6 юнитов умещается в одну волну) и крупными (Xfood 30+ — 3 волны
# вместо 5 при старом 6). Dodo IS rate-limit по факту больше, чем 12;
# если начнёт рвать коннекты — _with_retries поймает и повторит.
_MAX_PARALLEL = 12
