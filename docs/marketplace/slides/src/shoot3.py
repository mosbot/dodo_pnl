...
def ctx(p, w, h, mobile=False):
    ...
    c=b.new_context(viewport={'width':w,'height':h}, device_scale_factor=3, is_mobile=mobile, has_touch=mobile, locale='ru-RU', timezone_id='Europe/Moscow')
...
with sync_playwright() as p:
    if job=='bd': b,c=ctx(p,1440,900); pg=c.new_page(); board(pg,'board3.png'); b.close()
    if job=='fd': b,c=ctx(p,1440,900); pg=c.new_page(); fin(pg,'fin3.png'); b.close()
