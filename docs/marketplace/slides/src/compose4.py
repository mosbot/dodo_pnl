...
if which in('cover','all'):
    page("<h1>Все метрики каждой пиццерии — <em>на одном экране, с целями</em></h1>"
         "<div class='checks'><div class='chk'><i>✓</i>Выручка, заказы, кухня, доставка и рейтинги из Dodo IS — по каждой точке</div><div class='chk'><i>✓</i>Цели и цветовые зоны — сразу видно, где «в красном»</div><div class='chk'><i>✓</i>История метрик по месяцам и год к году</div><div class='chk'><i>✓</i>Пульс: вся сеть сегодня против прошлой недели</div></div>"
...
if which in('cards','all'): feat('Карточка месяца по каждой точке — <em>с целями и зонами</em>','Выручка по каналам, заказы, метрики кухни и доставки, рейтинги клиентов, РКО и РС. Красное, жёлтое, зелёное — против цели и прошлого года.',F+'frag-cards.png','../fin/f-1-cards.png',w=1168,top=322)
if which in('history','all'): feat('История метрик — <em>по месяцам, одним кликом</em>','Клик по пиццерии — все её метрики за выбранный период с трендом: где растёт, где проседает.',F+'frag-history.png','../fin/f-2-history.png',w=1100,top=322)
if which in('pulse','all'): feat('Пульс: вся сеть сегодня — <em>против прошлой недели</em>','Выручка каждой пиццерии, активные стопы, скорость кухни и доставки, месяц LFL. За 30 секунд понятно, куда звонить.',F+'frag-board.png','../fin/f-3-pulse.png',w=1168,top=322)
if which in('yoy','all'): feat('Год к году — <em>по месяцам и по каналам</em>','Закрытые месяцы хранятся, текущий обновляется ежедневно. Доставка, ресторан, самовывоз — против того же месяца прошлого года.',F+'frag-charts-yoy.png','../fin/f-4-yoy.png',w=1168,top=322)
if which in('pnl','all'): feat('Полный P&L по статьям — <em>если у вас PlanFact</em>','Себестоимость, ФОТ, маржинальная и чистая прибыль по каждой пиццерии и по сети. Без PlanFact — Lite-режим на данных Dodo IS.',F+'frag-pnl.png','../fin/f-5-pnl.png',w=1168,top=322)
