...
### Не держать транзакцию запроса через внешние вызовы
Инцидент 2026-09-25 (`docs/audits/pnl-pool-exhaustion-2026-09-25.md`): rolling
refresh сессии писал в транзакцию запроса → row-lock на строке сессии держался,
пока `/api/board` 20+ мин ждал Dodo IS; остальные запросы того же юзера
вставали за lock с занятым коннектом → пул 5+10 кончился → 500 у всех.
Теперь `touch_session_detached` и флаш активности — в своих коротких
транзакциях; доска делает `session.commit()` (отпускает коннект) перед
ожиданием лока и перед походом в Dodo IS, потолок сборки 60 с → 504. Правило:
ручка, которая ходит во внешний API, до похода коммитит/отпускает сессию и
ничего не пишет в транзакцию, которую держит через этот поход.
...
Месяц периода считается по правилу БОЛЬШИНСТВА ДНЕЙ (`_rating_month_for_period`):
неделя относится к месяцу, куда попало ≥4 её дней; РКО месяца = среднее таких
недель (4–5/мес), РС — 2 периода/мес.

**Скользящее среднее + цветовые зоны (миграция 0034).** Под значением РКО/РС
карточки выводится «среднее» (подпись `avg_label`): РКО — среднее rate последних
**12 недель**, РС — последних **6 проверок**. Период-НЕзависимое («последнее
доступное»): колонки `ops_metrics.rko_avg12w`/`rs_avg6` пишутся ТОЛЬКО при синке
текущего месяца (`_run_ops_sync`, окно ~110 дней, сортировка по `periodTo`),
а в ответ `/api/pnl` подтягиваются «последние непустые» через
`store.get_rating_trailing_latest` → `main._attach_rating_trailing` (вливает в
`project.ops`, поэтому одинаково на карточке любого месяца). Цветовые зоны
(`meta.zones=[redMax,yellowMax]`, рендерит `zoneCls` в `app.js`, применяется и к
значению, и к среднему/подстрокам): РКО/РС `[80,85]` (<80 красн, 80–85 жёлт,
≥85 зелён), РК `[4.7,4.8]` (<4,7 красн, 4,7–4,8 жёлт, ≥4,8 зелён). Бэкфилл
среднего для key1/3/4 сделан разово; новым тенантам заполнится при первом
current-month синке.

### Рейтинг клиентов (0..5) — СДЕЛАНО (Customer Rating API, миграция 0033)
Средняя оценка заказов из `/customer-feedback/customer-ratings` (generic-база
`api.dodois.io/customer-feedback`, `_feedback_base()`; `from`/`to` ≤31 дн,
«сегодня» недоступно → текущий месяц до вчера). Прямой диапазонный средний за
месяц. Колонки `ops_metrics.customer_rating` (общее, взвешено по числу оценок
зал+доставка) + `_dinein` + `_delivery` (None если оценок канала нет). Мета
`CUST_RATING` (digits 2, direction higher) с `subs` [зал, доставка] — фронт
(`app.js`) рендерит подстроку «зал X · доставка Y» под основным значением
(переиспользуемый механизм `meta.subs`). История залита разово (по 1 запросу на
месяц) для key4/key1/key3.
...
- **Маркетплейс Dodo IS — два приложения.** «Dodotool Касса» (`019e26f1…`,
  клиент `cnM4i`, тариф KASSA) и «Dodotool Финансы» (`01a072b7…`, стр.
  `01a072cc…`, клиент `cyuqZ`; тарифы FINANSY / PUL'S / FINANSY-I-PUL'S,
  триал 14 дн). Обе заполнены, не опубликованы.
  **Цены (ревизия 15.09, вариант «B»)**: Касса **2 490**, Финансы **1 490**,
  Пульс **990**, «Финансы и Пульс» **1 990** — ступени 1–5/6–15/16–30/31–45/46+
  по ×1,00/0,85/0,70/0,58/0,50. Сетка, аргументация и PDF —
  `docs/marketplace/listing-and-pricing-proposal.md` («Ревизия цен 2026-09-15»),
  генератор `docs/marketplace/slides/src/make_tariff_pdf.py`.
  **В кабинете цены применены 15.09** (Касса 2 490, Финансы 1 490 / Пульс 990 /
  пакет 1 990). Ступени в кабинете не задаются — «Спецены» это заявка площадке
  на конкретную сеть, сетку согласуем письмом.
